...
function __oah_install_env()
{
  ...
  echo "<= execute from environment script =>"
  ...
  if [ $? -eq 0 ];  then
    ...
    cp -r $env_base/host           $current_env 2>/dev/null
    #echo "Copying $env_base/provisioning   => $current_env"
    cp -r $env_base/provisioning   $current_env 2>/dev/null
    ...
    cp $env_base/oah-config.yml    $current_env 2>/dev/null
